...
export default function Navbar() {
  ...
  const navLinks = [
    ...
    { name: "College Projects", href: "#college-projects" },
    ...
    >
      <div className="px-6 md:px-8">
        <div className="flex items-center justify-between">
          <a href="#" className="flex items-center gap-3 xl:gap-4 group">
            ...
            <span
              className={`font-black text-xl md:text-2xl tracking-tight transition-colors duration-500 ${
                isScrolled ? "text-slate-900" : "text-white"
              }`}
            >
              Delta{" "}
              <span
                className={`transition-colors duration-500 ${isScrolled ? "text-blue-600" : "text-blue-400"}`}
              >
                Devs
              </span>
            </span>
          ...
          <div className="flex lg:hidden items-center gap-2">
            <button
              className={`p-2 transition-colors duration-500 ${
                isScrolled
                  ? "text-slate-900 hover:bg-slate-100 rounded-full"
      ...
      {isOpen && (
        <motion.div
          ...
          className="lg:hidden absolute top-full left-0 right-0 mt-4 bg-white/95 backdrop-blur-xl border border-white/50 rounded-3xl p-6 flex flex-col gap-5 shadow-[0_20px_40px_rgba(0,0,0,0.1)] overflow-hidden"
        >
          {navLinks.map((link) => (
            <a
              key={link.name}
